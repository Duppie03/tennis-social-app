...
document.addEventListener('DOMContentLoaded', ()=>{
    ...
    function createCourtCard(court) {
        ...
        const formatName = (playerObj)=>{
            ...
            if (nameParts.length === 1) // Single name (unlikely but handle it)
            return nameParts[0];
        ...
        try {
            ...
            if (data.success && data.csvContent) {
                ...
                if (guestsFromCSV.length > 0) {
                    ...
                    console.log(`\u{2705} Loaded ${guestsFromCSV.length} guests from CSV`);
                }
            }
        } catch (error) {
    ...
    async function exportGuestCSV() {
        try {
            ...
            if (readResponse.ok) {
                ...
                if (readData.success && readData.csvContent) existingGuests = parseGuestCSV(readData.csvContent);
            }
            ...
            if (state.guestHistory && state.guestHistory.length > 0) state.guestHistory.forEach((guest)=>{
                const key = guest.name.toLowerCase();
                const existing = guestMap.get(key);
                if (existing) // Update existing guest with newer data
                guestMap.set(key, {
                    name: guest.name,
            ...
            if (writeResponse.ok) {
                console.log(`\u{2705} Guest CSV exported successfully (${allGuests.length} guests)`);
                // Update state.guestHistory to match what's in CSV
                state.guestHistory = allGuests;
            } else console.error("\u274C Failed to export guest CSV:", writeResponse.statusText);
        } catch (error) {
    ...
    async function exportMemberToCSV(name, gender, type) {
        try {
            ...
            const data = await response.json();
            if (data.success) {
                if (data.duplicate) console.log(`\u{2139}\u{FE0F} Member already exists in CSV: ${name}`);
                else console.log(`\u{2705} Member added to CSV: ${name}`);
                return true;
            } else {
    ...
    playerTypeRadios.forEach((radio)=>{
        radio.addEventListener('change', (e)=>{
            ...
            if (phoneContainer) phoneContainer.style.display = isGuest ? 'flex' : 'none';
            if (emailContainer) emailContainer.style.display = isGuest ? 'flex' : 'none';
...
//# sourceMappingURL=public.7c7a35ae.js.map
